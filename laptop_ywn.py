...
gpu_log=[0] * (GPU_NUM * MODELS_PER_GPU)
...
def load_template(template_path):
    template=[]
    csv_files = list(pathlib.Path(template_path).glob("*.csv"))
    for csv_file in tqdm(csv_files, total=len(csv_files), desc="reading template", unit="files"):
...
def compare_template_image(template,images):
    distances=[100.0 for i in range(50)]
...
def run_app():
    ...
    a=executor.submit(predict, order_id, video_address)
...
def predict(arg1,arg2):     
    ...
    while(True):
        ...
        if ret:
            ...
            with torch.no_grad():
                if gpu_detect != -1:
                    model_yolov5, model_arcface = yolo_arcface_list[gpu_detect]
...
if __name__=="__main__":
    ...
    for gpu_index in range (GPU_NUM):
        for j in range(MODELS_PER_GPU):
            model_yolov5, model_arcface = load_yolov5_and_arcface_model(gpu_index, yolov5_weight_path, arcface_weight_path)
            yolo_arcface_list.append([model_yolov5, model_arcface])
