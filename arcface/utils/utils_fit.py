...
def fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, test_loader, lfw_eval_flag, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        with torch.no_grad():
            accuracy         = torch.mean((torch.argmax(F.log_softmax(outputs, -1), dim=-1) == labels).type(torch.FloatTensor))
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        with torch.no_grad():
            ...
            accuracy    = torch.mean((torch.argmax(outputs, dim=-1) == labels).type(torch.FloatTensor))
